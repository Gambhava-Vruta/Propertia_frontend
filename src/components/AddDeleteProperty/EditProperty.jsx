...
export default function EditPropertyPage() {
  ...
  useEffect(() => {
    ...
    const fetchPropertyData = async () => {
      setLoading(true);
      try {
        const data = await fetchWithAuth(`https://localhost:7117/api/properties/${id}`);
  ...
  const removeExisting = async () => {
    ...
    try {
      await fetchWithAuth(`https://localhost:7117/api/property-images/by-name/${imageToDelete.imageName}`
, { method: "DELETE" });
      setProperty(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://localhost:7117/api/properties/${id}`, {
        method: "PUT",
          ...
          <div className="image-preview-container">
            {property.existingImages.map((img, i) => (
              <div key={i} className="image-preview-wrapper">
                <img src={`https://localhost:7117/images/${img}`} alt="" className="image-preview" />
